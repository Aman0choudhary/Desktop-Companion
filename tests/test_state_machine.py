...
class StateMachineTests(unittest.TestCase):
    def test_greeting_flows_into_idle(self) -> None:
        machine = StateMachine(greeting_duration_seconds=2.0, sleep_after_seconds=10.0, hype_duration_seconds=2.0)
    ...
    def test_typing_switches_to_work(self) -> None:
        machine = StateMachine(greeting_duration_seconds=1.0, sleep_after_seconds=10.0, hype_duration_seconds=2.0)
        machine.initialize(0.0)
    ...
    def test_long_idle_switches_to_sleep(self) -> None:
        machine = StateMachine(greeting_duration_seconds=1.0, sleep_after_seconds=5.0, hype_duration_seconds=2.0)
        machine.initialize(0.0)
        machine.update(make_snapshot(now=1.2, idle_seconds=0.2, recent_keypresses=0, is_typing=False))
    ...
    def test_hype_returns_to_idle_after_duration(self) -> None:
        machine = StateMachine(greeting_duration_seconds=1.0, sleep_after_seconds=5.0, hype_duration_seconds=2.0)
        machine.initialize(0.0)
        machine.update(make_snapshot(now=1.2, idle_seconds=0.1, recent_keypresses=0, is_typing=False))
        machine.celebrate(2.0)
        transition = machine.update(make_snapshot(now=4.5, idle_seconds=0.2, recent_keypresses=0, is_typing=False))
        self.assertIsNotNone(transition)
        self.assertEqual(transition.current, NezukoState.IDLE)
